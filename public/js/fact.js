...
const Search = () => {
    ...
    const select = document.getElementById('selected').innerText;
    // console.log(select);
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState === 4){
            ...
            const parentElement = document.getElementById("factContent");
            let html = "";
            for (let i = 0; i < jsonObject['data'].length; i++) {
                const content = jsonObject['data'][i];
                html += `
                <a href="/?detailfact&id=${content.id}">
                    <div class="factcard">
                        <img src="${content.path_photo}" alt="" class="image">
                        <div class="facttext">
                            <h2>${content.title}</h2>
        ...
        xhttp.open('GET', `../../server/controller/auth/Fact.php?search=${search}&select=${select}`, true);
...
const showAll = () => {
    ...
    xhttp.open('GET', `../../server/controller/auth/Fact.php?show=${show}`, true);
    xhttp.send();
